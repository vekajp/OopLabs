...
namespace BackupsExtra.SaveChanges
{
    public class LocalChangeSaver : IChangeSaver
    {
        public void Update(BackupJob job)
        {
            ...
            job.Points.ToList().ForEach(point => CleanRestorePoint(point, jobPath));
            job.Points.ToList().ForEach(point => AddFilesToPoint(point, repository, jobPath));
        }
